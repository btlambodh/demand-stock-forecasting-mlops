...
def test_single_prediction():
    """Test single prediction with SageMaker-format features"""
    print("\n Testing single prediction endpoint...")
    try:
        # Use SageMaker-compatible input format
        test_data = {
...
def test_batch_prediction():
    """Test batch prediction endpoint with features"""
    print("\n Testing batch prediction endpoint...")
    try:
        # Create test data for batch prediction (SageMaker format)
        test_data = {
            "instances": [
                {
                    "Total_Quantity": 150.5,
...
def test_different_models():
    ...
    try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 1 <= price <= 1000:
                        ...
                        print(f"        Price seems extreme for {model_name}")
        ...
        if success_rate >= 0.8:
            ...
            print(f"     Partial fix - some models still have issues")
            return True
        else:
...
def test_edge_cases():
    ...
    if success_rate >= 0.75:
        ...
        print(f"     Most edge cases working")
        return True
    else:
...
def run_load_test():
    """Run a load test with SageMaker-format data"""
    print("\n Running load test...")
    try:
        test_data = {
            "Total_Quantity": 150.5,
        ...
        if successful_requests >= num_requests * 0.8:
            ...
            print(f"     Load test performance acceptable")
            return True
        else:
...
def test_sagemaker_compatibility():
    ...
    print("\n Testing SageMaker format compatibility...")
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"    SageMaker format test passed")
            ...
            if feature_count >= 80 and feature_count <= 100:
                ...
                print(f"     Unexpected feature count: {feature_count}")
            ...
            print(f"    SageMaker format test failed: {response.status_code}")
            if response.status_code == 500:
                try:
                    error_detail = response.json().get('detail', 'Unknown error')
                    print(f"   Error: {error_detail}")
        ...
        print(f"    SageMaker format test error: {e}")
        return False
...
def main():
    ...
    print(" Starting SageMaker-Synchronized API Test Suite")
    print("=" * 70)
    ...
    tests = [
        ...
        ("SageMaker Compatibility", test_sagemaker_compatibility),
        ("Provided Test Data", test_with_provided_test_data)
    ]
    
    passed = 0
    total = len(tests)
    critical_tests = ["Single Prediction", "Batch Prediction", "SageMaker Compatibility"]
    critical_passed = 0
    ...
    print("\n" + "=" * 70)
    print(" SageMaker-Synchronized API Test Results")
    print("=" * 70)
    print(f"Overall tests passed: {passed}/{total} ({(passed/total)*100:.1f}%)")
    print(f"Critical tests passed: {critical_passed}/{len(critical_tests)}")
    
    if critical_passed == len(critical_tests):
        print(" ALL CRITICAL TESTS PASSED! API IS AND WORKING! 🎉")
